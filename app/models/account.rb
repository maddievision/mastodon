...
class Account < ApplicationRecord
  ...
  validates :display_name, length: { maximum: 999 }, if: -> { local? && will_save_change_to_display_name? }
  validates :note, note_length: { maximum: 9999 }, if: -> { local? && will_save_change_to_note? }
